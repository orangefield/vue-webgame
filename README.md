...
8-5. context menu
8-6. 보호연산자
8-7. 재귀(caching, memoization, dynamic programming)
8-8. 끝

# MineSweeper.vue의 timer와 result는 store에 만들어둔 것을 mapState로 가져와서 사용 
# 기본적인 함수에 기능을 더 추가하기 위해 고차함수로 만듦, 함수를 확장하는 행위로 state만 인자로 갖던 함수에 row와 cell을 인자로 더 받아서 기존 함수를 확장한다
```